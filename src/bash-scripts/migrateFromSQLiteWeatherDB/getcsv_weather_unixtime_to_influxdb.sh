...
# influxdb用CSV出力 timestamp: unix epoch
get_csv() {
    device_name="$1"
    where_range="$2";
cat<<-EOF | sqlite3 "$PATH_WEATHER_DB" -csv
    SELECT
      td.name,measurement_time,temp_out,temp_in,humid,pressure
    FROM
...
# Need dateTime precision
# influx write --precision s -f t_weather_unixtime.csv  
